...
var TileType = {
  OUT_OF_BOUNDS : "$", // TODO: remove this tiletype...it doesn't make sense. Yes, it simplifies code, but it's hard to understand when you come back to this project after a while
  EMPTY         : ".",
  REGULAR       : "*",
  CONCRETE      : "#",
  DEACTIVATED   : "`",
...
// TODO: move game logic into game.js
draw = function() {
  screen.clear();
...
pollDrag = function( e ) {
  ...
  function move( from, to ) {
    ...
    to.forEach( function( cell, i ) { grid.set( cell.x, cell.y, new Tile( fromVals[i].type, fromVals[i].value - 1 ) ); } );
  }

  prune( selected[0] );
  if ( selected[0].type == TileType.EMPTY ) {
    update();
    draw();
    return;
  }
...
init = function() {
  ...
  grid = model.squareGrid( gridw, gridh, new Tile( TileType.EMPTY, 0 ), new Tile( TileType.OUT_OF_BOUNDS, -1 ) );

  // set up grid
  for ( i = 0; i < gridw * gridh; i++ ) {
    var val = Math.round( Math.random() * ( maxval - 2 ) + 2 );
    grid.setFlat( i, new Tile( TileType.REGULAR, val ) );
    if ( tiles[ val ] == null ) {
      tiles[ val ] = 0;
    }
    tiles[ val ]++;
  ...
  for ( i = 0; i < gridw * gridh; i++ ) {
    if ( Math.random() > 0.4 ) {
      if ( tiles[ grid.getFlat( i ).value ] > grid.getFlat( i ).value ) {
        ...
        grid.setFlat( i, new Tile( TileType.EMPTY, -1 ) );
      }
    }
  }
    ...
    if ( tiles[i] > i ) {
      ...
      for ( j = 0; j < gridw * gridh && tiles[i] > i; j++ ) {
        if ( grid.getFlat( j ).value == i ) {
          grid.setFlat( j, new Tile( TileType.EMPTY, -1 ) );
          tiles[i]--;
        }
      }
    }
  ...
  for ( i = 2; i <= maxval; i++ ) {
    if ( tiles[i] < i ) {
      ...
      while ( tiles[i] < i ) {
        ...
        if ( grid.getFlat( randIndex ).type == TileType.EMPTY ) {
          grid.setFlat( randIndex, new Tile( TileType.REGULAR, i ) );
          tiles[i]++;
        }
      }
    }
  ...
  if ( enabled[ TileType.LAVA ] ) {
    tiles = [];
    for ( i = 0; i < gridw * gridh; i++ ) {
      if ( grid.getFlat( i ).type == TileType.EMPTY ) {
        if ( Math.random() > 0.4 ) {
          var val = Math.round( Math.random() * ( maxval - 4 ) + 2 );
          grid.setFlat( i, new Tile( TileType.LAVA, val ) );
          if ( tiles[ val ] == null ) {
            tiles[ val ] = 0;
          }
          tiles[ val ]++;
    ...
    for ( i = 0; i < gridw * gridh; i++ ) {
      if ( Math.random() > 0.4 ) {
        if ( grid.getFlat( i ).type == TileType.LAVA && tiles[ grid.getFlat( i ).value ] > grid.getFlat( i ).value ) {
          tiles[ grid.getFlat( i ).value ]--;
          grid.setFlat( i, new Tile( TileType.EMPTY, -1 ) );
        }
      }
    ...
    for ( i = 2; i <= maxval; i++ ) {
      if ( tiles[i] > i ) {
        var count = tiles[i];
        for ( j = 0; j < gridw * gridh && tiles[i] > i; j++ ) {
          if ( grid.getFlat( j ).type == TileType.LAVA && grid.getFlat( j ).value == i ) {
            grid.setFlat( j, new Tile( TileType.EMPTY, -1 ) );
            tiles[i]--;
          }
        }
    ...
    for ( i = 2; i <= maxval; i++ ) {
      if ( tiles[i] < i ) {
        ...
        while ( tiles[i] < i ) {
          var randIndex = Math.round( Math.random() * ( gridw * gridh - 1 ) );
          if ( grid.getFlat( randIndex ).type == TileType.EMPTY ) {
            grid.setFlat( randIndex, new Tile( TileType.LAVA, i ) );
            tiles[i]++;
          }
        }
      }
